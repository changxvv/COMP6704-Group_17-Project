...
def dijkstra_shortest_path(network: Network, source: Node, target: Node, costs: Dict[tuple, float]):
    import heapq
    dist = {nid: float('inf') for nid in network.nodes}
    prev = {nid: None for nid in network.nodes}
    dist[source.id] = 0.0
    pq = [(0.0, source.id)]
    while pq:
        ...
        if u_id == target.id:
            break
        node_u = network.nodes[u_id]
        for v, e_id in node_u.succ.items():
            edge = network.edges[e_id]
            w = costs.get((edge.u.id, edge.v.id), edge.init_weight)
            nd = d + w
            if nd < dist[v]:
                dist[v] = nd
                prev[v] = u_id
                heapq.heappush(pq, (nd, v))
    if dist[target.id] == float('inf'):
        return [], float('inf')
    route_ids = []
    cur = target.id
    while cur is not None:
        ...
        cur = prev[cur]
    route_ids.reverse()
    route_nodes = [network.nodes[nid] for nid in route_ids]
    return route_nodes, dist[target.id]
